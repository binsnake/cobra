...
Dataset harness: `cargo build --release -p cobra-testkit --bin cobra-sweep`, then `cobra-sweep datasets/**/*.txt --bitwidth 64`. Gate = `unsafe=0` (no simplification that isn't equivalent to its input).
